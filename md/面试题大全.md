...
组件v-model:   v-on:modelValue="message"  v-bind:update:modelValue="updateModelValue"
...
原理：vue.js内部将DOM节点，抽象成一个个的VNode节点，keep-alive组件的缓存也是基于VNode节点，它将满足条件的组件在cache对象中缓存起来，重新渲染的时候再将VNode节点从cache对象中取出并渲染。
...
```
4、组件懒加载的方式：通过import函数实现按需加载组件，减少初始化加载时间,
5、使用computed计算属性缓存计算结果，避免重复计算,
6、合理使用keep-alive组件缓存动态组件，避免不必要的组件销毁和创建,
7、在频繁触发的事件，可以使用节流和防抖，减少事件的处理频率,
8、避免内存泄漏，在组件销毁时记得清理定时器、事件监听器,
9、图片方面可以使用图片懒加载的方式，以及精灵图
10、第三方组件按需引入，减小打包体积。

1、合理使用 v-if 和 v-show,
2、使用key确保组件的唯一性，避免不必要的DOM更新
3、如果是确保不变的数据，可以使用v-once执行确保只渲染一次,
```

### 20.首屏性能优化

- 配置路由懒加载，通过import函数配置路由对应的组件路由，并且使用webpackChunkName设置分割代码块的名称。
- 如果第三方组件可以按需导入。
...
浏览器缓存机制是通过将用户访问过的网页资源存储在本地设备上，加快页面加载速度并减少网络带宽的一种技术。

浏览器缓存状态：

1、未缓存：资源尚未被缓存

2、有效：资源已缓存且未过期，可以直接使用

3、无效：资源已过期

浏览器验证机制：通过HTTP头部中的ETag和Last-Modified字段来判断资源是否发生变化，如果资源未变化，则直接使用缓存，如果资源已变化，则重新从服务器获取最新版本。

缓存策略：

```css
强缓存：通过Expires字段控制，资源在指定时间后过期，浏览器不会再次向服务器验证。
协商缓存：通过Cache-Control字段控制，浏览器会向服务器发送请求头中的If-None-Match或If-Modified-Since字段来验证资源是否已更新。
```

如何利用缓存提高性能

1. **优化缓存策略**：设置合理的过期时间，对于静态资源（如图片、CSS文件），可以设置较长的过期时间（如一年），减少重复请求。
2. **利用HTTP头部控制缓存**：通过设置`Cache-Control`字段，指定资源的有效时间。
3. **减少不必要的请求**：通过压缩文件、减少重定向等方式减少网络延迟。
4. **预加载和预取**：在页面加载时主动加载关键资源，或根据用户行为预测未来可能访问的资源，并提前加载。
5. **定期清理缓存**：用户可以通过浏览器设置清理缓存，或通过设置过期时间或容量限制，自动移除旧的或不再使用的缓存
